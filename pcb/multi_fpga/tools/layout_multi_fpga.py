...
pcb/mic_array/tools/place_mics.py's placement-only precedent for the single-FPGA board).
...
Usage (from project root):
  python3 pcb/multi_fpga/tools/layout_multi_fpga.py
"""
...
# angular position, matching pcb/mic_array/tools/place_mics.py's precedent for the
...
TOOLS_DIR  = os.path.dirname(os.path.abspath(__file__))  # pcb/multi_fpga/tools
OUTDIR     = os.path.dirname(TOOLS_DIR)                   # pcb/multi_fpga
ROOT       = os.path.dirname(OUTDIR)                      # pcb
...
LOCAL_LIB   = os.path.join(ROOT, "libraries", "multi_fpga", "footprints.pretty")
