...
pcb/mic_array/tools/make_schematic.py's make_arm() (same 96-mic IM72D128 layout as
...
Usage (from project root):
  python pcb/multi_fpga/tools/make_schematic_multi_fpga.py
"""
...
_ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))  # pcb
sys.path.insert(0, os.path.join(_ROOT, "mic_array", "tools"))
...
OUTDIR  = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))  # pcb/multi_fpga
PROJECT = "multi_fpga"
...
# to unused analog-capable FMC pins in pcb/mic_array/tools/make_schematic.py's mic array
...
# ── helpers (mirrors pcb/mic_array/tools/make_schematic.py conventions) ───────────────────────
...
# is laid out as a carrier PCB (see pcb/multi_fpga/tools/layout_multi_fpga.py). The Cmod
...
# footprints (see pcb/libraries/multi_fpga/footprints.pretty/*.kicad_mod file
...
def make_cluster(idx):
    ...
    # pcb/multi_fpga/tools/layout_multi_fpga.py -- not modelled as a schematic symbol, same as
...
def main():
    ...
    print("  2. Module footprints assigned (DIP-48 socket / Adafruit_FT232H / TCXO_Can) -- see pcb/multi_fpga/tools/layout_multi_fpga.py")
